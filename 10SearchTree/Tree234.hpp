...
class Tree234 {
public:
    Node *rbTreeTo234Tree(RBTreeNode<int,string> *rootRB,RBTreeNode<int,string> *nil);
    Tree234(RBTree<int,string>& rbTree);
    Tree234() = default;
    ...
    void PrintStr(const char *file_name = nullptr);
private:
    /**
    ...
    void PrintNodeStr(std::ofstream &ofs, Node *node, int64_t parent_index,
                    int64_t index, int8_t parent_child_index);

    Node *root_{nullptr};  ///< root node of the tree
};

Node* Tree234::rbTreeTo234Tree(RBTreeNode<int,string> *rbNode,RBTreeNode<int,string> *nil)
{
    if (rbNode == nil)
        return NULL;
    Node* pNode234;
    // case1: 2-Node
    if (rbNode->_left->_col == BLACK && rbNode->_right->_col == BLACK){
        pNode234 = new Node(rbNode->_kv.first);
        pNode234->SetCount(1);
        pNode234->SetChild(0,rbTreeTo234Tree(rbNode->_left,nil));
        pNode234->SetChild(1,rbTreeTo234Tree(rbNode->_right,nil));
    }
    // csae2: 3-Node
    else if (rbNode->_left->_col == RED && rbNode->_right->_col == BLACK){
        pNode234 = new Node(rbNode->_left->_kv.first);
        pNode234->SetItem(1,rbNode->_kv.first);
        pNode234->SetCount(2);
        pNode234->SetChild(0,rbTreeTo234Tree(rbNode->_left->_left,nil));
        pNode234->SetChild(1,rbTreeTo234Tree(rbNode->_left->_right,nil));
        pNode234->SetChild(2,rbTreeTo234Tree(rbNode->_right,nil));
    }
    else if (rbNode->_left->_col == BLACK && rbNode->_right->_col == RED){
        pNode234 = new Node(rbNode->_kv.first);
        pNode234->SetItem(1,rbNode->_right->_kv.first);
        pNode234->SetCount(2);
        pNode234->SetChild(0,rbTreeTo234Tree(rbNode->_left,nil));
        pNode234->SetChild(1,rbTreeTo234Tree(rbNode->_right->_left,nil));
        pNode234->SetChild(2,rbTreeTo234Tree(rbNode->_right->_right,nil));
    }
    else{
        pNode234 = new Node(rbNode->_left->_kv.first);
        pNode234->SetItem(1,rbNode->_kv.first);
        pNode234->SetItem(2,rbNode->_right->_kv.first);

        pNode234->SetCount(3);
        pNode234->SetChild(0,rbTreeTo234Tree(rbNode->_left->_left,nil));
        pNode234->SetChild(1,rbTreeTo234Tree(rbNode->_left->_right,nil));
        pNode234->SetChild(2,rbTreeTo234Tree(rbNode->_right->_left,nil));
        pNode234->SetChild(3,rbTreeTo234Tree(rbNode->_right->_right,nil));
    }
    return pNode234;
}

Tree234::Tree234(RBTree<int,string>& rbTree)
{
    if (rbTree.root() == rbTree.nil())  return ;
    root_ = rbTreeTo234Tree(rbTree.root(), rbTree.nil());
 ...
 * @param file_name output file name, if nullptr then use "out.dot" as default
 */
void Tree234::PrintStr(const char *file_name) {
    if (!file_name) {
        file_name = "outstr.dot";
    }
    ...
    if (root_) {
        // print root node
        PrintNodeStr(ofs, root_, -1, index, 0);
        ...
        while (!q.empty()) {
            ...
            if (!node_info.node->IsLeaf()) {
                if (node_info.node->GetCount() > 0) {
                    PrintNodeStr(ofs, node_info.node->GetChild(0), node_info.index,
                            ++index, 0);
                    ni.node = node_info.node->GetChild(0);
                    ni.index = index;
                    q.push(ni);

                    PrintNodeStr(ofs, node_info.node->GetChild(1), node_info.index,
                            ++index, 1);
                    ni.node = node_info.node->GetChild(1);
                    ni.index = index;
                    q.push(ni);
                ...
                if (node_info.node->GetCount() > 1) {
                    PrintNodeStr(ofs, node_info.node->GetChild(2), node_info.index,
                            ++index, 2);
                    ni.node = node_info.node->GetChild(2);
                    ni.index = index;
                    q.push(ni);
                ...
                if (node_info.node->GetCount() > 2) {
                    PrintNodeStr(ofs, node_info.node->GetChild(3), node_info.index,
                            ++index, 3);
                    ni.node = node_info.node->GetChild(3);
                    ni.index = index;
                    q.push(ni);
 ...
 * nodes
 */
void Tree234::PrintNodeStr(std::ofstream &ofs, Node *node, int64_t parent_index,
                        int64_t index, int8_t parent_child_index) {
    assert(node);

    switch (node->GetCount()) {
        case 1:
            ofs << "node_" << index << " [label=\"<f0> " << static_cast<char>(node->GetItem(0) + 'A')
                << "\"]\n";
            break;
        case 2:
            ofs << "node_" << index << " [label=\"<f0> " << static_cast<char>(node->GetItem(0) + 'A')
                << " | <f1> " << static_cast<char>(node->GetItem(1) + 'A') << "\"]\n";
            break;
        case 3:
            ofs << "node_" << index << " [label=\"<f0> " << static_cast<char>(node->GetItem(0) + 'A')
                << " | <f1> " << static_cast<char>(node->GetItem(1) + 'A') << "| <f2> "
                << static_cast<char>(node->GetItem(2) + 'A') << "\"]\n";
            break;
